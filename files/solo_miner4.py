...
def logg(msg):
    # basic logging 
    logging.basicConfig(level=logging.INFO, filename="miner4.log", format='%(asctime)s %(message)s')  # include timestamp
    logging.info(msg)
...
def bitcoin_miner(t, restarted=False):
    ...
    nNonce = nounces.start4
    max_nonce = nounces.end4
    ...
    while True:
        ...
        # nonce = hex(random.randint(nNonce, max_nonce))[2:].zfill(8)
