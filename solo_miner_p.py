...
def bitcoin_miner(t, start_nonce=0, restarted=False):
    if restarted:
...
class CoinMinerThread(ExitedThread):
    def __init__(self, arg=None, start_nonce=0):
        super(CoinMinerThread, self).__init__(arg, n=start_nonce)
        self.start_nonce = start_nonce

    def thread_handler2(self, arg):
        self.thread_bitcoin_miner(arg, self.start_nonce)

    def thread_bitcoin_miner(self, arg, start_nonce):
        ctx.listfThreadRunning[self.n] = True
        check_for_shutdown(self)
        try:
            ret = bitcoin_miner(self, start_nonce)
...
def StartMining():
    ...
    # Create multiple miner threads
    for i in range(NUMBER_OF_MINERS):
        start_nonce = 1000000000 * (i + 1)
        miner_t = CoinMinerThread(None, start_nonce)
        miner_t.start()
        logg(f"[*] Bitcoin miner thread {i + 1} started with start_nonce {start_nonce}")
...
#     logg("[*] Subscribe thread started.")
#     time.sleep(4)
#     NUMBER_OF_MINERS = multiprocessing.cpu_count()
#
#     # Create multiple miner threads
#     for i in range(NUMBER_OF_MINERS):  # Replace NUMBER_OF_MINERS with the number of instances you want to run
#         miner_t = CoinMinerThread(None)
#         miner_t.start()
#         logg(f"[*] Bitcoin miner thread {i + 1} started")
#         processes.append(miner_t)
#
#     print('Bitcoin Miner started')
#
